...
Definir a primeira fatia funcional para criar e persistir uma Carteira mínima, independente de usuário e preparada para receber operações em mudanças futuras.
...
O sistema SHALL persistir somente `id`, `nome` e `dataCriacao` para cada Carteira nesta fatia. O identificador SHALL ser gerado pelo banco, e a Carteira MUST NOT possuir associação com usuário, ação, corretora, operação, posição, snapshot ou histórico nesta change.

#### Scenario: Persistência da Carteira mínima
- **WHEN** uma solicitação válida é concluída
- **THEN** o banco contém uma nova Carteira com identificador gerado, nome validado e data de criação gerada pela aplicação, sem registros ou associações de funcionalidades futuras

#### Scenario: Carteira independente de usuário no MVP
- **WHEN** uma Carteira é criada nesta fatia
...
O sistema SHALL responder à criação concluída com `201 Created`, SHALL incluir `Location: /carteiras/{id}` e SHALL devolver `CarteiraResponse` completo com `id`, `nome` e `dataCriacao`, sem expor a entidade JPA diretamente.
...
O schema necessário à Carteira SHALL ser evoluído pelo changelog Liquibase compartilhado entre PostgreSQL e H2. O Hibernate SHALL apenas validar o mapeamento com `ddl-auto=validate`, e a migração desta capability MUST criar somente a tabela e as constraints necessárias à Carteira.

#### Scenario: Banco sem a tabela Carteira
- **WHEN** a aplicação inicia sobre um banco compatível que já recebeu os changeSets anteriores
- **THEN** o Liquibase cria a estrutura de Carteira antes de o Hibernate validar o mapeamento
...
- **THEN** o sistema responde no formato `StandardError`, com `code` e `details` quando aplicável, e não cria Carteira
