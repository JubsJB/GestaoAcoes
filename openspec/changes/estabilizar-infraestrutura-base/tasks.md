...
- [x] 2.4 Alterar a política de schema do profile `dev` para `validate` e confirmar que nenhuma configuração persistente usa `create` ou `create-drop`.
...
- [x] 2.6 Revogar ou rotacionar operacionalmente a credencial PostgreSQL anteriormente exposta e registrar a confirmação da rotação, sem reescrever o histórico Git sem autorização específica.
...
- [x] 4.2 Executar a suíte automatizada e confirmar que o contexto Spring Boot carrega integralmente com H2 e o profile `test`.
- [x] 4.3 Revisar os arquivos rastreados e o build para confirmar a ausência de credenciais PostgreSQL ativas, o isolamento do `.env`, a ausência de H2 fora do escopo de teste e de políticas destrutivas para schema persistente.
