...
@Service
public class ResultadoRealizadoService {

    private static final Comparator<ResultadoRealizadoResponse> ORDEM_APRESENTACAO = Comparator
            .comparing(ResultadoRealizadoResponse::mercado)
            .thenComparing(ResultadoRealizadoResponse::ticker)
            .thenComparing(ResultadoRealizadoResponse::acaoId);

    private final CarteiraRepository carteiraRepository;
    private final OperacaoRepository operacaoRepository;
    private final CalculadoraPosicao calculadora;
    private final ResultadoRealizadoMapper mapper;

    public ResultadoRealizadoService(
            CarteiraRepository carteiraRepository,
            OperacaoRepository operacaoRepository,
            CalculadoraPosicao calculadora,
            ResultadoRealizadoMapper mapper
    ) {
    ...
    public List<ResultadoRealizadoResponse> listarPorCarteira(Long carteiraId) {
        ...
        List<ResultadoRealizadoResponse> resultados = new ArrayList<>(operacoesPorAcao.size());
        ...
        for (List<Operacao> operacoes : operacoesPorAcao.values()) {
            ...
            if (!resultado.possuiVenda()) {
                continue;
            }

            Acao acao = operacoes.get(0).getAcao();
            resultados.add(mapper.toResponse(acao, resultado.resultadoRealizado()));
        }

        resultados.sort(ORDEM_APRESENTACAO);
    ...
    private ApiException falhaDeReplay(Long carteiraId, FalhaReplay falha) {
        ...
        if (falha.tipo() == TipoFalha.CALCULO_FORA_DA_PRECISAO) {
            return new ApiException(
                    ...
                    "Cálculo do resultado realizado excede a precisão aprovada",
                    detalhes
            );
        }
        return new ApiException(
