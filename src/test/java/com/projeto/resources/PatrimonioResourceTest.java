...
class PatrimonioResourceTest {
    ...
    @Test
    void returnsEmptyPatrimoniosWithoutLocationForExistingEmptyPortfolio() throws Exception {
        Carteira carteira = carteiraRepository.saveAndFlush(portfolio("Carteira vazia"));

        mockMvc.perform(get("/carteiras/{id}/patrimonio", carteira.getId()))
                .andExpect(status().isOk())
                .andExpect(header().doesNotExist("Location"))
                .andExpect(jsonPath("$", Matchers.aMapWithSize(2)))
                .andExpect(jsonPath("$.carteiraId").value(carteira.getId()))
                .andExpect(jsonPath("$.patrimonios", Matchers.hasSize(0)));
    }

    @Test
    void returnsNotFoundFromCentralizedHandler() throws Exception {
        mockMvc.perform(get("/carteiras/{id}/patrimonio", Long.MAX_VALUE))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404))
                .andExpect(jsonPath("$.path").value(
                        "/carteiras/" + Long.MAX_VALUE + "/patrimonio"
                ));
    }

    @Test
    void consolidatesBrlAndFractionalUsdSeparatelyInCurrencyOrder() throws Exception {
        ...
        mockMvc.perform(get("/carteiras/{id}/patrimonio", carteira.getId()))
                .andExpect(status().isOk())
                .andExpect(header().doesNotExist("Location"))
                .andExpect(jsonPath("$.patrimonios", Matchers.hasSize(2)))
                .andExpect(jsonPath("$.patrimonios[0]", Matchers.aMapWithSize(2)))
                .andExpect(jsonPath("$.patrimonios[0].moeda").value("BRL"))
                .andExpect(content().string(Matchers.containsString(
                        "\"patrimonioAtual\":4250.000000000000"
                )))
                .andExpect(jsonPath("$.patrimonios[1].moeda").value("USD"))
                .andExpect(content().string(Matchers.containsString(
                        "\"patrimonioAtual\":112.205000000000"
                )))
                .andExpect(jsonPath("$.patrimonios[0].dataHoraCotacao").doesNotExist())
                .andExpect(jsonPath("$.patrimonioTotal").doesNotExist());
    ...
    void reflectsPartialSaleOmitsTotalSaleAndUsesOnlyNewOpenCycle() throws Exception {
        ...
        mockMvc.perform(get("/carteiras/{id}/patrimonio", carteira.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.patrimonios", Matchers.hasSize(1)))
                .andExpect(jsonPath("$.patrimonios[0].moeda").value("BRL"))
                .andExpect(content().string(Matchers.containsString(
    ...
    @Test
    void currentQuoteChangesPatrimonyWithoutChangingAccountingPosition() throws Exception {
        Carteira carteira = carteiraRepository.saveAndFlush(portfolio("Carteira cotação"));
        Acao acao = acaoRepository.saveAndFlush(action(
                "PETR4", Mercado.BRASIL, Moeda.BRL, "30.000000"
        ));
        save(carteira, acao, TipoOperacao.COMPRA, "100", "32", "2026-08-01", 1);

        mockMvc.perform(get("/carteiras/{id}/patrimonio", carteira.getId()))
                .andExpect(content().string(Matchers.containsString(
                        "\"patrimonioAtual\":3000.000000000000"
                )));
        ...
        mockMvc.perform(get("/carteiras/{id}/patrimonio", carteira.getId()))
                .andExpect(content().string(Matchers.containsString(
                        "\"patrimonioAtual\":3550.000000000000"
                )));
        mockMvc.perform(get("/carteiras/{id}/posicoes", carteira.getId()))
                .andExpect(jsonPath("$[0].precoMedio").value(32.0))
                .andExpect(jsonPath("$[0].custoPosicao").value(3200.0))
                .andExpect(jsonPath("$[0].resultadoNaoRealizado").value(350.0));
    }
